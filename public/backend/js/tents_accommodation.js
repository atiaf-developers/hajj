...
var ids = [];
var created_at;
var TentsAccommodation = function () {
    ...
    var handleReport = function () {
        $('.btn-report').on('click', function () {
            ...
            var url = config.admin_url + "/tents_accommodation";
            var params = {};
    ...
    var handleAddOrRemoveItem = function () {
        ...
        $('.add-suite').on('click', function () {
            ...
            //console.log(tents);
            for (var x = 0; x < tents.length; x++) {
                var obj = tents[x];
                var disabled = '';
                var selected_tents = getSelectedTents();
                if (selected_tents.indexOf(obj.id) != -1) {
                    disabled += 'disabled';
                }
    ...
    var getSelectedTents = function () {
        var tents = [];
        $('select[name="suite"]').each(function () {
            if ($(this).val()) {
                tents.push(parseInt($(this).val()));
            }
        });
        return tents;
    }
    ...
    var getlounges = function (ele, suite) {
        ...
        setTimeout(function () {
            $.get('' + config.admin_url + '/tents_accommodation/lounges/' + suite, function (data) {
                var html = '';
...
//                },
            ...
            $('#regForm').submit(function () {
                ...
                $.ajax({
                    url: config.admin_url + "/tents_accommodation",
                    type: 'POST',
                    dataType: 'json',
                    data: formData,
                    async: false,
                            ...
                            if (step == 2) {
                                ...
                                if (tents.length > 0) {
                                    ...
                                    for (var x = 0; x < tents.length; x++) {
                                        var Obj = tents[x];
                                        html += '<div class="md-checkbox has-success col-md-4">' +
                                                '<input type="checkbox" id="' + Obj.id + '-' + Obj.number + '" name="tents[]" value="' + Obj.id + '" class="md-check">' +
                                                '<label for="' + Obj.id + '-' + Obj.number + '">' +
                                                '<span class="inc"></span>' +
                                                '<span class="check"></span>' +
                                                '<span class="box"></span>' + Obj.number + ' ( ' + Obj.available + ' ) ' + '</label>' +
    ...
    return {
        ...
        delete: function (t) {
            ...
            setTimeout(function () {
                $.ajax({
                    url: config.admin_url + '/tents_accommodation/delete',
        ...
        notify: function (ele) {
            ...
            setTimeout(function () {
                $.ajax({
                    url: config.admin_url + "/tents_accommodation/notify",
                    type: 'POST',
                    dataType: 'json',
                    data: {
